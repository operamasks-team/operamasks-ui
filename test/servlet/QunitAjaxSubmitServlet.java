	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (typeParam != null && !"".equals(typeParam)) {
			if (JSON.equals(typeParam)) {
				...
				PrintWriter writer = response.getWriter();
				writer.write("<div>content from server<script>var unitTestVariable1 = true;</script><script>var unitTestVariable2 = true;</script><script>if(undefined == " +
						"window.scriptCount) {scriptCount = 1;} else {scriptCount ++};</script></div>");
			}
		} else {
			response.setContentType("text/html");
			PrintWriter writer = response.getWriter();
