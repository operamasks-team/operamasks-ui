...
test("height", function() {
	...
	equal(el.height() , el.parent().height(), '初始化指定[height:"fit"]组件高度填充父容器');
	
	el = $("#tab-option-height-number").omTabs({height:'300px'});
	equal(el.height() , el.height() , '初始化指定[height:"300px"]组件高度则为300px');
});
...
test("tabHeight", function() {
	...
	el = $("#tab-option-default").omTabs();
	equal($(".om-tabs-headers li" , el).height() , omTabs_defaults.tabHeight , 'tabHeight的默认值为25px' );
	
	el = $("#tab-option-switchMode-tabHeight").omTabs({tabHeight: 30});
	equal($(".om-tabs-headers li" , el).height() , 30 , '初始化指定[tabHeight:30]');
});
...
test("tabWidth", function() {
	...
	el = $("#tab-option-default").omTabs();
	equal($(".om-tabs-headers li" , el).find("a").width() , omTabs_defaults.tabWidth , 'tabWidth的默认值为80px' );
...
test("width", function() {
	...
	equal(el.width() , el.parent().width(), '初始化指定[width:"fit"]组件宽度填充父容器');
	
	el = $("#tab-option-width-number").omTabs({width:'300px'});
	equal(el.width() , el.width() , '初始化指定[width:"300px"]组件宽度则为300px');
});
