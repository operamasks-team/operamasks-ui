...
asyncTest("reload setDataSource", function() {
	expect(6);
	
	el = $("#tab-method-reload").omTabs({lazyLoad:true});
	var remote = $(".om-tabs-panels .om-panel-body:last" , el).eq(0),
		local = $(".om-tabs-panels .om-panel-body" , el).eq(0);
	
	el.omTabs("setDataSource" , {index:0 , content:'new content'});
	notEqual(local.html() , "new content" , "本地取数setDataSource方法并不会马上生效");
	el.omTabs("reload" , 0);
	equal(local.html() , "new content" , "本地取数setDataSource执行后需要reload一下才生效");
	setTimeout(function(){
	...
	setTimeout(function(){
		...
		el.omTabs("setDataSource" , {index:2 , content:'new content' , url:'./remote.data2'});
	} , 1000);
	setTimeout(function(){
		equal(remote.html() , REMOTE_DATA , "远程取数setDataSource方法并不会马上生效");
		el.omTabs("reload" , 2);
	} , 1500);
	setTimeout(function(){
		equal(remote.html() , REMOTE_DATA2 , "远程取数setDataSource执行后需要reload一下才生效，而且setDataSource中url优先于content");
		el.remove();
		start();
	} , 2000);
});
