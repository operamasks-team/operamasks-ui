( function($) {
	...
	function compareStyleValue(element, expect, message) {
		...
		if(positionOfSemicolon != -1) {
			deepEqual(actualValue.substring(0, positionOfSemicolon).split(":"), expectValue, message);
		} else {
	...
	test("{ height:fit}", function() {
		...
		equal(element.omAccordion("option", "height"), '100%', "accordion's height is 'fit'");
	...
	test("{ width:fit }", function() {
		...
		equal(element.omAccordion("option", "width"), '100%', "accordion's width is 'fit'");
