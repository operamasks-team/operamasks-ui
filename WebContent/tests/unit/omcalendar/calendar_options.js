( function($) {
	...
	test("dateFormat", function() {
		...
		$('.om-state-nobd', element[0].calendar.con).click();
		$('button.ct-ok', element[0].calendar.con).click();
		value = element.val();
		defaultValue = $.omCalendar.formatDate(new Date(), 'yy-mm-dd H:i:s');
		equal(value, defaultValue, '不设置dateFormat且显示时间时应该默认格式是yy-mm-dd H:i:s');
	...
	test("disabled", function() {
		...
		var dropListVisibility = element[0].calendar.con.css('visibility');
	...
	test("disabledDays", function() {
		...
		$('div.om-dbd>a:eq(6)', element[0].calendar.con).click();
		//点击下拉框里第一行第7个日期（星期6）
	...
	test("disabledFn", function() {
		...
		$('div.om-dbd > a:contains("1"):eq(0)', element[0].calendar.con).click();
		//点击下拉框里1号
	...
	test("maxDate", function() {
		...
		$('div.om-dbd > a:contains("3"):eq(0)', element[0].calendar.con).click();
		//点击下拉框里本月3号
	...
	test("pages", function() {
		...
		equal(element[0].calendar.con.children().size(), 3, '下拉框里应该一次显示3个月');
	});
	test("popup", function() {
		var element = $('.popup div').omCalendar({});
		ok(element[0].calendar.con !== undefined, 'popup=false时可以把日历控件渲染在非input容器上');
	});
	...
	test("showTime", function() {
		...
		var hasTimeInput = $('button.ct-ok', element[0].calendar.con).size() > 0;
		...
		hasTimeInput = $('button.ct-ok', element[0].calendar.con).size() > 0;
		strictEqual(hasTimeInput, true, 'showTime=true后下拉框里应该显示出选择时间的输入框');
		$('button.ct-ok', element[0].calendar.con).click();
		defaultValue = $.omCalendar.formatDate(new Date(), 'yy-mm-dd H:i:s');
		equal(element.val(), defaultValue, 'showTime=true后选择当前日期后输入框里应该显示出时间');
	...
	test("startDay", function() {
		...
		var startDay = $('.om-whd > span:eq(0)', element[0].calendar.con).html();
		equal(startDay, '日', '默认情况下下拉框里第一天应该是周日');
		element = $('.startDay input:eq(1)').omCalendar({
			startDay : 4
		});
		startDay = $('.om-whd > span:eq(0)', element[0].calendar.con).html();
