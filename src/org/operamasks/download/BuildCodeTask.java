...
public class BuildCodeTask extends Task {
	...
	public void execute() throws BuildException {
		...
		// 代码合并后的CSS文件
		File concatCssFile = new File(sequenceFolder + "/themes/default/operamasks-ui.css");
		// 代码合并且压缩后的CSS文件
		File concatCssMinFile = new File(sequenceFolder + "/themes/default/operamasks-ui.min.css");
		...
		f = new FileName();
		// 添加om-theme.css
		f.setName(basePath + "/themes/default/om-theme.css");
		fl.addConfiguredFile(f);
		...
		srcDir.setDir(new File(basePath + "/themes/default/images/"));
		srcDir.setIncludes("*.*");
		c.setTodir(new File(sequenceFolder + "/themes/default/images/"));
		c.addFileset(srcDir);
		c.execute();
		...
		for (Resource r : orderlyResouces) {
			if (r.isIncluded() && r.getCssFile() != null) {
				f = new FileName();
				f.setName(basePath + r.getCssFile());
				fl.addConfiguredFile(f);
				if(r.getImgDir() != null){
					// 添加组件样式的图片
					c = new Copy();
					c.setProject(prj);
					srcDir = new FileSet();
					srcDir.setDir(new File(basePath + r.getImgDir()));
					srcDir.setIncludes("*.*");
					c.setTodir(new File(sequenceFolder + r.getImgDir()));
					c.addFileset(srcDir);
					c.execute();
				}
			}
